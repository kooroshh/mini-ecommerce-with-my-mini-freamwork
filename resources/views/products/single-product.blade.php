        ...
        <div class="lg:w-7/12 w-full">
            ...
            <div class="mt-10 divide-y">
                ...
                <div class="flex gap-8 *:whitespace-nowrap *:font-medium *:text-sm *:py-3 *:cursor-pointer">
                    <a id="commentsBtn" href="/products/{{ $product->slug }}" class="text-indigo-600 border-b-2 border-indigo-600">
                        Comments
                    </a>
                    ...
                    @if ($comments)
                        ...
                        @foreach ($comments as $comment)
                            ...
                            <div class="text-gray-500 text-sm flex flex-col">
                                ...
                                <div class="py-5 flex">
                                    <img class="bg-gray-100 rounded-full size-10" src="{{ image($comment->image, "users") }}" alt="">
                                    <div class="ml-2">
                                        <h3 class="text-gray-900 font-medium">{{ $comment->name }}</h3>
                                        <p>{{ explode(' ', $comment->created_at)[0] }}</p>
                                    </div>
                                </div>
                                <div class="w-full pb-5">

                                    <p class="bg-gray-50 w-full p-2 rounded-md break-words whitespace-pre-wrap">{{ $comment->body }}</p>
                    ...
                    @if (auth()->check())
                        <form class="space-y-6" action="/products/comment/add" method="post" novalidate>
                            ...
                            <div>
                                ...
                                <div class="mt-2">
                                    <textarea class="text-sm py-1.5 px-3 w-full min-h-20 rounded-md bg-white outline outline-1 -outline-offset-1 outline-gray-300" name="body" id="body">{{ $old('body') }}</textarea>
                                </div>
                                @if ($errors->has('body'))
                                    <p class="text-xs text-red-500 mt-1 pl-2.5">{{ $errors->first("body") }}</p>
                                @endif
                            </div>
        ...
        <div class="lg:w-5/12 w-full px-2">
            ...
            <div class="space-y-5 divide-y-2">
                ...
                <div>
                    ...
                    <p class="text-gray-500 my-6 w-full bg-gray-50 p-2 rounded-md break-words whitespace-pre-wrap">{{ $product->description }}</p>
